...
class KioskPostsTest extends WP_UnitTestCase {
  ...
  function test_kiosk_posts_short_should_have_the_correct_number_of_entries() {
    ...
    $this->assertContains( 'kiosk-posts__slider__slide', $content, 'Return all posts when no tags provided' );

    // Test empty tag
    $content = do_shortcode( '[kiosk-posts tags=""]' );
    $this->assertContains( 'kiosk-posts__slider__slide', $content, 'Return all posts when empty string passed on tags' );

    // Test with tags
    $content = do_shortcode( '[kiosk-posts tags="Kiosk"]' );
    $this->assertContains( 'kiosk-posts__slider__slide', $content, 'Return all posts with kisok tags' );

    // Test with multipe tags
    $content = do_shortcode( '[kiosk-posts tags="Kiosk,SOS,SSS,kisok,abc,linux,windows"]' );
    $this->assertContains( 'kiosk-posts__slider__slide', $content,'Return all posts with kisok Kiosk,SOS,SSS,kisok,abc,linux,windows' );

    // Test with wrong tags
    $content = do_shortcode( '[kiosk-posts tags="no tags with this name"]' );
    $this->assertNotContains( 'kiosk-posts__slider__slide', $content, 'Do not return any posts' );
    ...
    $this->assertContains( 'kiosk-posts__slider__slide', $content, 'Return the default images when tags are not present' );
